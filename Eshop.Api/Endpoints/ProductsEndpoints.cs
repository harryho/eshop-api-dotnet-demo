...
public static async Task<CreatedAtRoute<ProductDtoV1>> CreateProduct(IProductsRepository repository, CreateProductDto productDto)
    ...
    public static async Task<Results<NoContent, NotFound>> UpdateProduct(IProductsRepository repository, int id, UpdateProductDto product)
    {
        var existingProduct = await repository.GetAsync(id);
        if (existingProduct is null)
        {
    ...
    public static async Task<NoContent> DeleteProduct(IProductsRepository repository, int id)
    {
        var existingProduct = await repository.GetAsync(id);
        if (existingProduct is not null)
        {
